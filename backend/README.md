...
| **Spring WebFlux (WebClient)** | Boot-managed | Gọi HTTP tới AI service (Python/FastAPI) & Meta Graph/Threads API |

> **AI service (sinh nội dung)**: tác vụ tạo nội dung chạy **bất đồng bộ** (NFR-04) — API trả về job ngay,
> worker nền gọi AI service rồi cập nhật trạng thái để FE poll (`PENDING → RUNNING → SUCCESS/FAILED`).
...
| AI Service | `AI_SERVICE_BASE_URL` (dev `http://localhost:8000`), `AI_SERVICE_TIMEOUT_SECONDS` (mặc định `60`) — endpoint dịch vụ AI sinh nội dung |
